...
function didKeyToPublicKey (did) {
  ...
  if (bytes[0] === 0x80 && bytes[1] === 0x24) {           // P-256 (compressed SEC1)
    ...
    const xe = Buffer.from(x.toString(16).padStart(64, '0'), 'hex')
    const ye = Buffer.from(y.toString(16).padStart(64, '0'), 'hex')
    return { alg: 'ES256', key: createPublicKey({ key: { kty: 'EC', crv: 'P-256', x: b64u(xe), y: b64u(ye) }, format: 'jwk' }) }
  }
  return null
}
...
export function verifySelfIssuedJwt (token) {
  try {
    ...
    // suite: sub, iss, client_id present, identical, a did:key URI
    if (!claims.sub || claims.sub !== claims.iss || claims.sub !== claims.client_id) return null
    if (!claims.exp || claims.exp * 1000 < Date.now()) return null
    if (!claims.iat) return null
    const resolved = didKeyToPublicKey(claims.sub)
    if (!resolved || resolved.alg !== header.alg) return null
    const data = Buffer.from(h + '.' + p)
    const sig = Buffer.from(s, 'base64url')
    const ok = resolved.alg === 'EdDSA'
      ? cryptoVerify(null, data, resolved.key, sig)
      : cryptoVerify('SHA256', data, { key: resolved.key, dsaEncoding: 'ieee-p1363' }, sig)
